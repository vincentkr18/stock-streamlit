...
def initialize_session_state():
    ...
    if "data_loaded" not in st.session_state:
        st.session_state.data_loaded = set()
...
def main():
    ...
    with st.sidebar:
        ...
        for ticker in tickers_to_remove:
            ...
            if ticker in st.session_state.data_loaded:
                st.session_state.data_loaded.remove(ticker)
            if ticker in st.session_state.analyzer.data:
                del st.session_state.analyzer.data[ticker]
            st.rerun()
        ...
        if st.button("🔄 Load/Refresh Data"):
            ...
            for i, ticker in enumerate(st.session_state.tickers):
                ...
                if st.session_state.analyzer.fetch_data(ticker, period):
                    st.session_state.data_loaded.add(ticker)
    ...
    available_tickers = [
        t for t in st.session_state.tickers if t in st.session_state.data_loaded
    ]
    ...
    for ticker in available_tickers:
        if ticker in st.session_state.analyzer.data:
            ...
            dates = st.session_state.analyzer.data[ticker].index
    ...
    with col1:
        if st.button("1W"):
            st.session_state.start_date = current_date - timedelta(weeks=1)
            st.session_state.end_date = current_date
    with col2:
        if st.button("1M"):
            st.session_state.start_date = current_date - timedelta(days=30)
            st.session_state.end_date = current_date
    with col3:
        if st.button("3M"):
            st.session_state.start_date = current_date - timedelta(days=90)
            st.session_state.end_date = current_date
    with col4:
        if st.button("6M"):
            st.session_state.start_date = current_date - timedelta(days=180)
            st.session_state.end_date = current_date
    with col5:
        if st.button("1Y"):
            st.session_state.start_date = current_date - timedelta(days=365)
            st.session_state.end_date = current_date
    with col6:
        if st.button("2Y"):
            st.session_state.start_date = current_date - timedelta(days=730)
            st.session_state.end_date = current_date
    with col7:
        if st.button("3Y"):
            st.session_state.start_date = current_date - timedelta(days=1095)
            st.session_state.end_date = current_date
    with col8:
        if st.button("4Y"):
            st.session_state.start_date = current_date - timedelta(days=1460)
            st.session_state.end_date = current_date
    with col9:
        if st.button("5Y"):
            st.session_state.start_date = current_date - timedelta(days=1825)
            st.session_state.end_date = current_date
    ...
    if available_tickers:
        ...
        sample_data = st.session_state.analyzer.data[sample_ticker]
    ...
    for i, ticker in enumerate(available_tickers):
        with ticker_tabs[i]:
            if ticker not in st.session_state.analyzer.data:
                st.warning(f"No data available for {ticker}")
                continue

            # Get current price info
            current_data = st.session_state.analyzer.data[ticker].iloc[-1]
            prev_data = st.session_state.analyzer.data[ticker].iloc[-2]
            ...
            with col5:
                st.metric(
                    "52W Range",
                    f"${st.session_state.analyzer.data[ticker]['Low'].min():.2f} - ${st.session_state.analyzer.data[ticker]['High'].max():.2f}",
                )
            ...
            fig1 = st.session_state.analyzer.create_stock_price_chart(
            ...
            # Chart 2: MACD
            fig2 = st.session_state.analyzer.create_macd_chart(
                ticker, start_date, end_date
            )
            if fig2:
            ...
            # Chart 3: RSI
            fig3 = st.session_state.analyzer.create_rsi_chart(
                ticker, start_date, end_date
            )
            if fig3:
